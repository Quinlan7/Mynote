...
> 在Linux和Unix系统中，文件和目录的权限字符串由10个字符组成，用来表示文件或目录的权限。这些字符的含义如下：
>
> 1. 第一个字符：文件类型
>    - `-` 表示这是一个普通文件。
>    - `d` 表示这是一个目录。
...
>    - `-` 表示没有相应的权限。
>
> 这些权限字符的排列顺序是`rwxrwxrwx`，其中前三个字符表示所有者的权限，接下来的三个字符表示所属组的权限，最后的三个字符表示其他用户的权限。例如，权限字符串`-rw-r--r--` 表示一个普通文件，所有者有读和写权限，所属组和其他用户只有读权限。
>
> 此外，还有其他一些特殊权限位可以在权限字符串中显示，如 SUID、SGID 和粘滞位等。这些特殊权限位通常显示在执行权限的位置，用字母"S"、"s" 或 "t" 表示。
...
```
chmod --reference=gspass01.dbf gs
```

修改 gs 权限与 gspass01.dbf 相同
