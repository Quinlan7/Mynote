...
**流量控制**：限制业务访问的QPS（每秒查询数），避免服务因流量的突增而故障。
...
> 信号量隔离：通过限制 tomcat 中的每个服务的线程数量实现隔离（这个好）
>
> 线程池隔离：为每个服务创建独立的线程池（损耗大）
>
> 流量整形：使波动的流量变为稳定的流量
  ...
  cloud: 
    sentinel:
      transport:
        ...
        # 只有你的sentinel服务和微服务不在同一个服务器时需要配置，sentinel默认去本机探测连接它的微服务
        port: 8719  # sentinel客户端端口
        client-ip: 172.16.31.104 # sentinel客户端IP
```
...
warm up也叫**预热模式**，是应对服务冷启动的一种方案。请求阈值初始值是 ==maxThreshold / coldFactor==，持续指定时长后，逐渐提高到maxThreshold值。而==coldFactor的默认值是3==.
...
- 排队等待：==可以达到流量整形的效果==，请求会进入队列，按照阈值允许的时间间隔依次执行请求；如果请求预期等待时长大于超时时间，直接拒绝
...
![image-20240305210411481](https://raw.githubusercontent.com/Quinlan7/pic_cloud/main/img/202403052104148.png)
