...
如果我们采用phone：手机号这个的数据来存储当然是可以的，但是如果把这样的敏感数据存储到redis中并且从页面中带过来毕竟不太合适，所以==我们在后台生成一个随机串token，然后让前端带来这个token就能完成我们的整体逻辑了==
...
**主动更新：**我们可以手动 调用方法把缓存删掉，通常用于解决缓存和数据库不一致问题
...
* 方法1：根据指定的key查询缓存，并反序列化为指定类型，利用缓存空值的方式解决缓存穿透问题
...
public class CacheClient {
    ...
    public CacheClient(StringRedisTemplate stringRedisTemplate) {
	 ...
	 /**
     * 首先这是一个泛型方法：需要在返回类型前声明你要用到的泛型名称
     * @param type Class是一个泛型类，在作为参数传递泛型类的时候其实可以指定泛型的具体类型也可以不指定，
     *             指定的话可以避免因为类型出现转换等错误。这里指定为 泛型方法声明的类型
     * @param dbFallback Function<ID, R> 这是一个函数类型，传递的是一个函数
     */
...
==这种算法其实还是可以看出来自增的数量的（通过后几位），这个算法很一般==
...
> 线程表示为 UUID + 线程ID
>
> + 首先UUID为 static 修饰的 类变量，在程序启动的时候就会被初始化。所以在同一个JVM上的程序使用的是同一个 static 变量，这个 UUID 是用来区别不同的JVM的。
> + 这个线程ID 是用来区分线程的。
> + 如果只使用线程ID可能会有 在两台机子上的不同线程，拥有相同的线程ID
...
> 就是 拿锁 判断锁 删除锁 并非原子性的，所以有可能有各种错误：
>
> + 使用lua脚本执行多条redis命令，可以实现原子性
...
> java 编程技巧与API：
>
> + 将lua脚本写成一个文件，方便以后修改。读取文件需要使用IO流，如果释放一次锁，读取一次浪费IO资源，做成 静态变量 只在类加载的时候读取一次，节省IO资源。
>
> + 如果一个静态变量的初始化很多步骤，写在静态代码块中，静态变量 静态代码块 都只在类加载的时候执行一次。
> + ClassPathResource 类：用于读取资源文件，默认的路径就是在 Resouces 下的文件，所以这里只有文件名
> + DefaultRedisScript\<Long\>：Redis脚本类
> + RedisTemplate.execute 方法，用于执行lua脚本
> + Collections.singletonList()：通于把一个变量 变为 只有一个元素的LIST集合
...
> redission的锁重试机制：利用了redis 的发布订阅模式 和 信号量，而非自旋可以有效降低cpu资源的使用，当发现锁被占用后 会先等待而不是自旋（一直占用资源），当检测到我们要获取的锁被释放了之后才会继续后边的逻辑。
>
> redission的超时续约机制：利用 watchDog，设置了一个定时任务，每隔一段时间（releaseTime/3），重置超时时间。releaseTime 默认时间为 30s。
>
> 超时续约相当于没有过期时间吗？ 不！
>
...
![image-20240429185006088](https://raw.githubusercontent.com/Quinlan7/pic_cloud/main/img/202404291850593.png)
...
### 5.6 分布式锁-redission锁的总结

> 可重入：利用 hash 结构记录线程id和重入次数
>
> 可重试：利用 信号量 和 PubSub（发布订阅）功能实现等待、唤醒，获取锁失败的重试机制
>
...
>
> 如何让查询出来的顺序，和你提供的顺序一致。ORDER BY FIELD(id, 5, 1)
...
> Redis中的GEO数据类型是一种扩展数据类型，适用于处理与地理位置相关的数据，特别适合于LBS（基于位置的服务）应用。 该数据类型的底层结构是**基于Sorted Set实现的，通过GeoHash编码来保存经纬度信息**。
...
当我们点击美食之后，会出现一系列的商家，商家中可以按照多种排序方式，我们此时关注的是距离，这个地方就 需要使用到我们的GEO，向后台传入当前app收集的地址(我们此处是写死的) ，以当前坐标作为圆心，同时绑定相同的店家类型type，以及分页信息，把这几个条件传入后台，后台查询出对应的数据再返回。
