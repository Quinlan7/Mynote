...
# 2.运行es
- docker run --name es -d -p 9200:9200 -p 9300:9300  -e "discovery.type=single-node" -e xpack.security.enabled=false elasticsearch:8.12.0
...
> 如果你要查询的字段中有 text 类型的话，那么 mutli_match 会自动用你的text类型的分词器进行分词，然后用分词结果查询你的 text 字段，用不分词的查询条件查询别的 字段
...
```java
@Configuration
public class ElaticSearchDataSource extends AbstractElasticsearchConfiguration {
    @Override
    @Bean
...
@Test
    public void testQueryId() throws IOException {
        GetRequest getRequest = new GetRequest("fruit","3");
        //返回值：查询响应对象
...
> 可以当作查询饿通用模板，只需要修改 QueryBuilders 对象
...
@Test
    public void testMatchAll() throws IOException {
        ...
        System.out.println("最大的分"+search.getHits().getMaxScore());
...
> 就是利用 QueryBuilders 对象，构造查询条件

```java
@Test
    public void testMatchCondition() throws IOException {
        //1 term 查询
        testMatchConditionMode(QueryBuilders.termQuery("description","西瓜"));
        //2 range 查询
        testMatchConditionMode(QueryBuilders.rangeQuery("price").gt(0).lt(10));
        //3 prefix 前缀
        testMatchConditionMode(QueryBuilders.prefixQuery("description","西"));
        //4 wildcard 通配符查询 ？一个字符 *任意多个字符
        testMatchConditionMode(QueryBuilders.wildcardQuery("description","西?"));
        //5 ids 多个指定 id 查询
        testMatchConditionMode(QueryBuilders.idsQuery().addIds("1").addIds("2"));

        //6 multi_match 多字段查询
        testMatchConditionMode(QueryBuilders.multiMatchQuery("好吃").field("title").field("description"));
...
@Test
    public void testSearch() throws IOException {
        ...
        sourceBuilder.query(QueryBuilders.termQuery("description","好吃"))
                ...
                .size(2)        //每页显示条数 默认返回 十条
                .sort("price", SortOrder.ASC)  //指定排序字段 参数一：根据那个字段排序 参数二：排序方式
                .fetchSource(new String[]{"title"},new String[]{}) //参数一：包含字段数组 参数二：排除字段数组
...
> 其实和 Query 用法是一样的，只是再调用一个 postFilter，里面的对象也是 QueryBuilder。
>
> Filter意义：
>
> ​	在数据量很大的时候进行 查询的时候，先使用 Filter 过滤条件，可以加速查询速度，因为正常 Query 是会对每个结果进行打分的，以确定每个结果的匹配度。但是 Filter 的过程是不会计算得分的，所以先用 Filter 过滤，不匹配选项会加速查询过程。
...
> 对象存入 ES，直接把对象变为Json格式的String，通过
>
> new ObjectMapper().writeValueAsString(products)

```java
@Test
    public void testObjectOp() throws IOException {
        Products products = new Products();
        products.setId(10);
        products.setTitle("小浣熊");
        products.setPrice(1.78);
        products.setDescription("好吃啊");

        //录入es   使用writeValueAsString将对象输出为String类型的Json字符串
        IndexRequest indexRequest = new IndexRequest("fruit");
        indexRequest.id(String.valueOf(products.getId()))
...
//2 取出对象
        ...
        for (SearchHit hit : hits) {
            System.out.println(hit.getSourceAsString());
            Products products1 = new ObjectMapper().readValue(hit.getSourceAsString(), Products.class);
...
> 其实对于这个的使用，主要还是要熟悉再kibana上的使用，返回结构的获取，其实和kibana上的一样。
...
@Test
    public void testAggs() throws IOException {
        ...
        //获取对应名字的聚合  用相应的解析类 接收，因为price是Double，并且是term聚合
...
@Test
    public void testAggsFunction() throws IOException {
        ...
        ParsedSum priceSum = aggregations.get("price_sum");
